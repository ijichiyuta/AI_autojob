...
6. **案件情報は秘密情報として扱う。** 2026-09-12 にクラウドワークスとNDAを締結した。
   NDA第2条により「クライアントの情報」「クライアントが発注する仕事に関する情報」は秘密情報。
   第3条4項で複製は業務遂行上必要な範囲で認められるが、**複製したものも秘密情報として
   善管注意義務をもって管理する義務がある**（第6条）。違反すると損害賠償（第8条）。
   → **ダッシュボードを無認証で公開しない。** `dashboard/proxy.ts` のBasic認証を外さないこと。
   `DASHBOARD_USER` / `DASHBOARD_PASSWORD` 未設定だと localhost 以外からは 503 を返す。

7. **ログインを自動化しない。** `npm run login` はブラウザを開くだけで、ID・パスワードは入力しない。
